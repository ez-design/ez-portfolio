...
async function moviePosterFnc(movieName, movieOpenDate) {
  for (let i = 0; i < movieName.length; i++) {
    ...
    const moviePosterValue = {
      key: `?ServiceKey=84UPEK1N3PE2QUP8ZE31`,
...
function detailTagFnc(movieLength) {
  const $movieNoDataHide = document.querySelectorAll(".sec02 .inner .detail li:not(.movie__name)");
  const $movieNoData = document.querySelector(".sec02 .inner .plot");

  if(movieDetailArray[movieLength].openingDate === "no data"){ // openingData의 값이 no data 일때 
